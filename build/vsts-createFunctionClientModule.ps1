...
$parentModule = 'PoshCISAKevFeed'
if (-not $ModuleName) { $ModuleName = 'PoshCISAKevFeed.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\PoshCISAKevFeed'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\PoshCISAKevFeed.psd1"
$prereqHash = @{
